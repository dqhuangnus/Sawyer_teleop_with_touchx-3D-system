...
impl ObjectiveMaster {
    ...
    pub fn relaxed_ik(chain_lengths: &[usize]) -> Self {
        ...
        for i in 0..num_chains {
            ...
            weight_priors.push(200.0);   // pos_weight: 50 → 200，强化位置精度
            ...
            weight_priors.push(50.0);    // rot_weight: 20 → 50，强化姿态精度（pos:rot = 200:50 = 4:1）
        ...
        objectives.push(Box::new(MinimizeVelocity));     weight_priors.push(0.1);  // 0.7 → 0.1，降低速度约束
        objectives.push(Box::new(MinimizeAcceleration)); weight_priors.push(0.1);  // 0.5 → 0.1，降低加速度约束
        objectives.push(Box::new(MinimizeJerk));         weight_priors.push(0.05); // 0.3 → 0.05，降低加加速度约束
